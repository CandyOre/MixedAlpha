...
Allocation propa_emp (vector<UtilityQueryable*> agents, int agents_amount, int goods_amount) {
    Allocation allocation(goods_amount);
    ...
    int last_agent = *agents_set.begin();
    auto last_agent_inst = agents[last_agent];
    allocation.set_div_next_piece(1, last_agent + 1);
    for (int i = good_idx; i < goods_amount; i++) {
        allocation.set_ind_partition(i, last_agent + 1);
        bool stop = false;
        for (int j = 0; j < good_idx; j++) {
            if (
                allocation.get_utility(last_agent + 1, last_agent_inst)
                + last_agent_inst->get(j) * last_agent_inst->alpha()
                >= total_utility[last_agent] / agents_amount
            ) {
                stop = true;
                break;
            }
    ...
    for (int i = good_idx; i < goods_amount; i++) {
        allocation.set_ind_partition(i, (i - good_idx) % agents_amount + 1);
    }
    return allocation;
}
